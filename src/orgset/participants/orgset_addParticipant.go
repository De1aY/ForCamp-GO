...
func AddParticipant(token string, participant Participant, ResponseWriter http.ResponseWriter) bool {
	if orgset.CheckUserAccess(token, ResponseWriter) && checkAddParticipantData(participant, ResponseWriter) {
...
func addParticipant_Organization(participant Participant) *conf.ApiError{
	Query, err := src.NewConnection.Prepare("INSERT INTO users VALUES(?,?,?,?,?,?,?,?)")
	if err != nil {
		log.Print(err)
		return conf.ErrDatabaseQueryFailed
	}
	...
	Query.Close()
	Query, err = src.NewConnection.Prepare("INSERT INTO participants(login) VALUES(?)")
	if err != nil {
		log.Print(err)
		return conf.ErrDatabaseQueryFailed
	}
...
func getTeamNameById(id int64) (string, *conf.ApiError){
	if id == 0{
		...
		Query, err := src.NewConnection.Query("SELECT name FROM teams WHERE id=?", id)
		if err != nil {
			log.Print(err)
			return "", conf.ErrDatabaseQueryFailed
		}
