...
func deleteParticipant_Organization(login string) *conf.ApiError{
	Query, err := src.NewConnection.Prepare("DELETE FROM users WHERE login=? AND access='0'")
	if err != nil {
		log.Print(err)
		return conf.ErrDatabaseQueryFailed
	}
	...
	Query, err = src.NewConnection.Prepare("DELETE FROM participants WHERE login=?")
	if err != nil {
		log.Print(err)
		return conf.ErrDatabaseQueryFailed
	}
