...
func SetHeaders_Main(w http.ResponseWriter){
	...
	w.Header().Set("Content-Security-Policy", "default-src 'self' https://api.forcamp.ga; font-src 'self' https://fonts.gstatic.com https://cdnjs.cloudflare.com/; img-src 'self' https://www.google-analytics.com/ data:; script-src 'unsafe-eval' 'self' https://www.google-analytics.com/analytics.js https://cdnjs.cloudflare.com 'unsafe-inline'; style-src 'unsafe-inline' 'self' https://fonts.googleapis.com https://cdnjs.cloudflare.com")
	w.Header().Set("X-XSS-Protection", "1; mode=block")
	w.Header().Set("X-Frame-Options", "SAMEORIGIN")
	w.Header().Set("X-Content-Type-Options", "nosniff")
	w.Header().Set("Strict-Transport-Security", "max-age=63072000; includeSubDomains; preload")
