...
func TestLoginAndPasswordHandler(t *testing.T) {
	Request, err := http.NewRequest("GET", "/signin?login=test_1&password=test", nil)
	if err != nil {
		t.Fatal(err)
	}
...
func TestTokenVerification(t *testing.T){
	Request, err := http.NewRequest("GET", "/token?token=dGVzdF8xMTQ4OTQzMDk4NTU1NzcwMDY3OTE5NDc3Nzk0MTDPg-E1fu-4vfFUKFDWbYAH1iDkBQtXFdyD9Kkh02zpzkfQ0TxdhfKw_4MY0od-7C9juTG9R0F6gaU4Mnr5J9o-", nil)
	if err != nil{
		t.Fatal(err)
	}
	Recorder := httptest.NewRecorder()
