...
$('#submit').click(function () {
    $.get('https://api.forcamp.ga/token.get', {login: $('#login').val(), password: $('#password').val()}, function (data) {
        if (data.code == 200) {
            $.cookie('token', data.token, { expires: 366, path: '/', secure: true });
            ...
            notie.alert({type: 3, text: data.message, time: 3});
        }
    }, "json");
});